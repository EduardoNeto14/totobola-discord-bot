...
module.exports = {
	...
	description : "Limpar jornada do Best Of",
			...
			if (Object.keys(args).length == 1) {
				...
				db.get(`select estado from jornadas where jornada = "bestof${args[0]}"`,  (err, estado) => {
					...
					if ((typeof estado !== "undefined") && (estado.estado === "y")) {
						db.run(`DROP TABLE IF EXISTS bestof${args[0]}`);
						db.run(`DROP TABLE IF EXISTS bestof${args[0]}_games`);
						db.run("DELETE FROM jornadas where jornada=?", `bestof${args[0]}`);
