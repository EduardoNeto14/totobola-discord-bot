...
module.exports = {
	name : "startbestof",
	description : "Começar jornada do Best Of",
	execute(message, args) {
		const messageEmbed = new Discord.MessageEmbed().setTitle("Best Of");
		if (["MrRed", "Rouxinol Expansivo"].includes(message.author.username)) {
			
			if (Object.keys(args).length <= 1) {
				
				messageEmbed.setColor("#969C9F");
				messageEmbed.setDescription("Este comando dá início a uma jornada do Best Of. Para iniciar, siga o exemplo:\n!startbestof\n{jornada}\nJogo 1\nJogo 2\nJogo n");
				...
				let db = new sqlite3.Database(dbpath, sqlite3.OPEN_READWRITE, (err) => {
					...
					db.get(`select jornada from jornadas where jornada = "bestof${args[0]}"`, (err, j) => {
						...
						if ((isNaN(parseInt(args[0], 10))) || (typeof j !== "undefined")) {
							...
							let sql = `CREATE TABLE bestof${args[0]} (jogador text`;
							let placeholder = "(";
							...
							db.run(sql);
							sql = sql.replace("jogador text, ", "").replace(", pontuacao int", "").replace(`bestof${args[0]}`, `bestof${args[0]}_games`);
							...
							db.run(sql, (err) => {
								...
								db.run(sql, args.slice(1, Object.keys(args).length), (err) => {	
									...
									db.run(sql + ";", [`bestof${args[0]}`, Object.keys(args).length - 1, 'y'], (err) => {
