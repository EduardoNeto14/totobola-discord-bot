...
function finalizar(messageEmbed, message, args, db) {
	...
	db.run(`drop table bestof${args[0]}_games`, (err) => {
		if (err) {
		...
		db.run(`update jornadas set estado = "n" where jornada = "bestof${args[0]}"`, (err) => {
			...
			db.all(`select jogador, pontuacao from bestof${args[0]} where pontuacao = (select max(pontuacao) from bestof${args[0]})`, (err, winners) => {
				...
				winners.forEach( (winner) => {
					...
					db.run(`insert into vencedoresbestof (jogador, pontuacao, jornada) values ("${winner.jogador}", ${winner.pontuacao}, "${args[0]}")`);
				
				});

				db.get(`select avg(pontuacao) from bestof${args[0]}`, (err, media) => {
				...
				if (Object.keys(args).length <= 1) {
					...
					let db = new sqlite3.Database(dbpath, sqlite3.OPEN_READWRITE, (err) => {
						...
						if (err) {
							...
							db.get(`select estado from jornadas where jornada = ?`, [`bestof${args[0]}`], (err, estado) => {
								...
								if ((typeof estado !== "undefined") && (Object.values(estado)[0] === 'y')) {
									db.all(`select * from bestof${args[0]}`, (err, progs) => {
										...
										if (err) {
											...
											db.get(`select jogos from jornadas where jornada = ?`, [`bestof${args[0]}`], (err, n_jogos) => {
												...
												if ( (args.length - 1) == n_jogos.jogos) {
													...
													for (const prog of Object.entries(progs)) {
														...
														db.run(`update bestof${args[0]} set pontuacao = ${pontuacoes[`${prog[1]["jogador"]}`]} where jogador = "${prog[1]["jogador"]}"`, (err) => {
															if (err) {
																messageEmbed.setColor("#969C9F");
																messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
																message.channel.send(messageEmbed);
																console.log("Erro ao atualizar a pontuação na tabela bestof0");
																return;
															}
														

															db.run(`update bestof set jornadas = 																\
																	((select jornadas from bestof where jogador = "${prog[1]["jogador"]}") + 1), 					\
																	totalBestOf = ((select totalBestOf from bestof where jogador = "${prog[1]["jogador"]}") 			\
																...
																if (err) {
																	...
																	messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
																	message.channel.send(messageEmbed);
																	console.log("Erro ao atualizar a tabela bestof");
																	return;
																}
															
																db.run(`update total set totalBestOf = 																\
