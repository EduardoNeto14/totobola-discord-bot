...
function finalizar(messageEmbed, message, args, db) {
	...
	db.run(`drop table champions${args[0]}_games`, (err) => {
		if (err) {
		...
		db.run(`update jornadas set estado = "n" where jornada = "champions${args[0]}"`, (err) => {
			...
			db.all(`select jogador, pontuacao from champions${args[0]} where pontuacao = (select max(pontuacao) from champions${args[0]})`, (err, winners) => {
				...
				winners.forEach( (winner) => {
					...
					db.run(`insert into vencedoreschampions (jogador, pontuacao, jornada) values ("${winner.jogador}", ${winner.pontuacao}, "${args[0]}")`);
				
				});

				db.get(`select avg(pontuacao) from champions${args[0]}`, (err, media) => {
					console.log("absdajknsba");
					if (err) {
						messageEmbed.setColor("#969C9F");
						messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
						message.channel.send(messageEmbed);
						...
						if (err) {
							...
							db.get(`select estado from jornadas where jornada = ?`, [`champions${args[0]}`], (err, estado) => {
								...
								if ((typeof estado !== "undefined") && (Object.values(estado)[0] === 'y')) {
									db.all(`select * from champions${args[0]}`, (err, progs) => {
										...
										if (err) {
											...
											db.get(`select jogos from jornadas where jornada = ?`, [`champions${args[0]}`], (err, n_jogos) => {
												...
												if ( (args.length - 1) == n_jogos.jogos) {
													...
													console.log("Tendência dos jogos:" , tendencia_resultado);
													...
													for (const prog of Object.entries(progs)) {
														...
														console.log("Começar pontuação de:", prog[1]);
														console.log("Pontuação: ", pontuacoes[`${prog[1]["jogador"]}`]);	
														...
														db.run(`update champions${args[0]} set pontuacao = ${pontuacoes[`${prog[1]["jogador"]}`]} where jogador = "${prog[1]["jogador"]}"`, (err) => {
															if (err) {
																messageEmbed.setColor("#969C9F");
																messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
																message.channel.send(messageEmbed);
																console.log("Erro ao atualizar a pontuação na tabela champions0");
																return;
															}
														

															db.run(`update champions set jornadas = 																\
																	((select jornadas from champions where jogador = "${prog[1]["jogador"]}") + 1), 					\
																	totalChampions = ((select totalChampions from champions where jogador = "${prog[1]["jogador"]}") 			\
																...
																if (err) {
																	...
																	messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
																	message.channel.send(messageEmbed);
																	console.log("Erro ao atualizar a tabela champions");
																	return;
																}
															
																db.run(`update total set totalChampions = 																\
																	...
																	if (err) {
																		...
																		messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
																		message.channel.send(messageEmbed);
																		console.log("Erro ao atualizar a tabela total");
																		return;
																	}	
																});
															});
