...
module.exports = {
	name : "vencedoreschampions",

	description : "Mostrar vencedores do Champions",

	execute(message, args) {

        const messageEmbed = new Discord.MessageEmbed().setTitle("Vencedores Champions");
        ...
        let db = new sqlite3.Database(dbpath, sqlite3.OPEN_READONLY, (err) => {
            ...
            if (Object.keys(args).length === 0) {
                ...
                db.all("select * from vencedoresChampions", (err, vencedores) => {
                    if (err) {
                        messageEmbed.setColor("#969C9F");
                        messageEmbed.addField("Erro", "Erro na conexão da base de dados. Contacte alguém responsável");
                        message.channel.send(messageEmbed);
                ...
                db.all(`select * from vencedoresChampions where jogador = "${message.mentions.users.first().username}"`, (err, vitorias) => {
